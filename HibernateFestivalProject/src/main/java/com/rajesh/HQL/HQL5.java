...
//           }
        ...
        Query<Festival> q = s.createQuery(
        		//SELECT f FROM Festival f ORDER BY f.id
        	    "SELECT f from Festival f order by f.name",
        	    Festival.class
        	);
         // q.setParameter("month", "October");
          //q.setParameter("month2", "September");
       //   List<Festival[]>  data=q.executeUpdate();
        	List<Festival> data = q.getResultList();
